...
export function assistantTurnSystemPrompt(profile: UserProfile, plan: WeekPlan): string {
  ...
  return (
    ...
    "- swap_meal: replace a meal with a specific dish; requires dish. Give day + mealType to change ONE meal ('swap Monday breakfast for cottage cheese pancakes'); OMIT day to set that dish on EVERY day ('pancakes for breakfast every day' → mealType:breakfast, no day). Never do one day and claim 'every day'. By DEFAULT the app keeps that day on the user's macro targets by adjusting the other meals' portions (like a nutritionist fitting a treat in) — you don't ask for that, it's automatic. Set preserveMacros:false ONLY when the user signals they don't care this time ('cheat day', 'treat', 'whatever, I don't care about macros'). You never need to compute macros — the app does the math.\n" +
    ...
    "- substitute_ingredient: the user has run out of an ingredient, or can't get one ('I don't have greek yogurt', 'no chicken, what else?', 'can I use something instead of quinoa?'). Requires ingredient. day + mealType only if they name a meal. Changes nothing. The app checks the swap against their diet and allergies and computes the macro cost. Never suggest a substitute yourself.\n" +
    "- symptom_check: the user reports how they FEEL ('i'm always tired', 'my nails keep breaking', 'i keep getting sick', 'muscle cramps'). Pass their words verbatim in `symptom`. Changes nothing. The app decides which nutrients the symptom is associated with, checks them against the user's real week, and handles anything urgent. You must NEVER map a symptom to a nutrient, never name a deficiency, never mention a supplement, and never diagnose — just call the tool and let the app answer.\n" +
    "- lock_meal: the user wants a meal to stay put ('never change my sunday roast', 'keep monday breakfast', 'always give me oats on tuesday', 'pin friday dinner'). Requires day + mealType. Changes nothing right now; the app puts that meal back every time it rebuilds the week.\n" +
    "- unlock_meal: undo a pin ('you can change sunday again', 'unpin monday breakfast'). Requires day + mealType.\n" +
    "- rate_meal: the user says what they THOUGHT of a dish ('that salmon was incredible', 'the tofu was awful, never again', 'monday's breakfast was ok'). Requires rating (1-5) plus either dish, or day + mealType. Map their words to the number: hated/never again = 1, didn't like = 2, fine/ok = 3, liked = 4, loved/incredible = 5. Changes nothing right now; the app favours what they love and stops planning what they hate. Contrast with 'i don't like mushrooms', which is an ingredient the user never wants (update_profile + excludeFoods), and with log_meal, which records what they ATE, not what they thought of it.\n" +
    "- hydration: the user asks about water or fluid ('how much water should i drink?', 'am i drinking enough?', 'what's my water target'). Changes nothing. If the message gives a weight or an activity level, pass weightKg / activity; otherwise pass NO fields — the app already knows their weight if they ever worked out their targets, and asks if it does not. Never state a litre figure yourself.\n" +
    "- scale_portions: the user wants MORE or LESS food, not different food ('i'm still hungry', 'that's way too much', 'make tuesday's portions bigger', 'smaller servings please'). Requires portionChange: much_smaller | smaller | bigger | much_bigger. Add day, or day + mealType, only if they named one; no day means the whole week. NEVER pass a number or a factor — the app decides how much and checks it against the calorie floor. Contrast with update_profile targetCalories, which is a permanent change to what they should be eating.\n" +
    "- undo: the user wants the last change reversed ('undo', 'undo that', 'actually put it back', 'revert that', 'never mind, change it back'). No fields. It must be the ONLY operation you emit — never undo and then do something else in the same turn. The app steps back one change; if there is nothing to undo it says so.\n" +
    ...
    "'I want pancakes for breakfast every day' → [{tool:swap_meal, mealType:breakfast, dish:'pancakes'}] (no day = every day; never do one day and say 'every day')\n" +
