...
cleared too: whole-batch swaps (`f50345e`) + Today parity (`30500de`) + live cross-screen re-render
(`11f1085`). Remaining minor: only per-batch locks (benign — batch is deterministic). Kimi K3 beta needs an owner key.
...
An autonomous session on the deterministic engine. Shipped: **macro accuracy** (`chooseRecipe` ranks
by macro-density fit first, so fat/carbs land on target, with honest disclosure); **condition→
nutrient generation** (`conditions.ts` + `selectConditionAwareWeek`, tested but UNWIRED pending the
ask-vs-auto-apply UX call); **`what_if` seedable determinism** (a `withSeed` seam, `b19c27d`, so a
simulation is reproducible while ordinary generation stays fresh); and **`gramsFor` unit coverage**
(cup/oz/kg/lb/l for pasted quantities).

Then **every module got an adversarial review**, fixing real bugs the ~440-test suite had missed: an
allergen leak (pesto/hummus/Caesar), `log_meal` silent data-loss, `agentTools` crash-on-bad-args,
`computeTargets`/`gramsFor` input gaps, `composeReply` duplicate-note, feed substring over-match,
`streak` UTC-vs-local, and — the one that mattered most — an **SSRF on `/api/import`** where a
redirect bypassed `isSafePublicUrl` (fixed `88df7ae`: re-validate the final URL + a tighter host
guard; residual DNS-rebinding documented in-code). Each fix carries a regression test. New lessons
from it are **38–40** below.

Finally this file was **de-duplicated**: its entire body had been duplicated and the two halves had
diverged, with a corruption at the seam that split lesson 35 so NEITHER copy held it intact. The
corrupted older copy was removed and lesson 35 reconstructed from both.
...
35. **It is shell EXPANSION that eats backticks, not heredocs — quote the delimiter.** Writing
    Markdown containing `code spans` through `node -e "…"` mangled the content or died on command
    substitution, five times across two sessions, because bash expands backticks and `$(…)` inside
    double quotes before node ever sees the string. The fix is not "avoid the shell": a heredoc
    with a **quoted** delimiter (`<<'EOF'`) passes every byte through untouched, and writing the
    replacement text to a file and splicing it in with a tiny script is reliable. An UNquoted
    delimiter (`<<EOF`) expands and fails exactly like the double-quoted case. Write/Edit remain
    the safest option when the content is going straight into a file unchanged.
36. **A cleanup command killed by a timeout leaves the mess committed.** A scratch probe was
    written to `scripts/`, used, and removed with `rm -f` chained onto the end of a long-running
    command. The command hit the 10-minute cap and was killed at the *node* step, so the `rm` never
    ran — and the scratch file went out in the next commit, into the directory that holds the test
    suite and real tooling, where an unexplained file reads as something worth running. Two rules
    out of it: put scratch work in the scratchpad directory, not in the repo, so a missed cleanup
    is harmless; and never chain cleanup onto a command that can be killed — a step that must
    happen gets its own invocation.
37. **A test that asserts on a random draw is a coin flip, and it will eventually pay out.** Two
    gate failures appeared together — `regenerate_week really does change the plan (control)` and
    `undo reports the plan as changed` — and the very next run passed both with identical code.
    The signature identifies the cause exactly: those two fail while `undo restores the exact plan`
    PASSES only if the regenerated week came back **identical** to the one before it, because then
    undo genuinely has nothing to change. The engine picks at random among near-tied recipes
    (`pickRecipe` in `recipeDb`), so an identical week is possible.
    **What the measurement adds, and what it cost:** 312 isolated runs produced **zero** identical
    weeks, and a separate probe confirmed **no operation mutates the profile or plan it is handed**
    — so the tempting "shared `BASE` drifts across the suite and collapses the recipe pool" theory
    is dead. The rate is simply below 1-in-312, which is why it read as an inexplicable one-off and
    burned a session proving the engine was fine. **Rare is not the same as impossible, and a
    suite that runs hundreds of times will find a 1-in-1000 event.** Fixed by retrying regeneration
    a bounded 20 times: a genuinely broken `regenerate_week` still fails, because it fails all 20,
    and a pool that has collapsed to one candidate still fails too — the retry removes the dice
    without hiding a real defect. **The general form: when a test depends on randomness, assert the
    property that must hold every time, or retry until the precondition is met — never assert that
    a single draw came out lucky.** Do not re-derive the above; the probes are gone, the answer is
    here.
38. **A green suite is not a correct module — every adversarial per-module review found a real bug.**
    At ~440 passing tests, a one-module-at-a-time review still turned up a genuine defect in EACH of
    ~7 modules, always in an edge the tests didn't exercise: a snack on a 3-meal day, a redirect, a
    0-weight, a mixed-number quantity, a duplicate op. Passing tests prove the paths you thought of;
    the review is for the paths you didn't. Worth the cost even on a well-tested engine.
39. **An SSRF allowlist that checks only the pasted string is bypassed by a redirect.** `isSafePublicUrl`
    validated the URL the user pasted, but `fetchHtml` followed redirects, so a public link that
    302'd to `169.254.169.254` or `127.0.0.1` fetched internal content anyway. Re-validating the
    FINAL url after redirects + tightening the host guard (bare no-dot hosts, `.internal`, 0.0.0.0/8,
    CGNAT, decimal-encoded IPv4) closes the string-level holes; the COMPLETE fix is connect-time IP
    validation via a custom undici dispatcher (residual: DNS rebinding, and the request fires once).
    A guard is only as strong as the LAST thing that actually opens the socket.
40. **A doc that only grows can rot silently — and the corruption can survive in every copy.** This
    file had its entire body DUPLICATED, the two halves had diverged, and the seam corruption split
