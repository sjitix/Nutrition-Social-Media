...
**The two egg halves sit together, and the rim lever moved to make that possible.** §3's
`one piece resting against the rim` exists to force something out of the tidy centre — but pointing
it at the egg (`one egg half resting against the rim`) pushed the halves to opposite ends of the
bowl, which is not how a halved egg is served. The lever now points at an avocado wedge, and the
egg is described explicitly: `cut in half with the two halves nestled side by side touching each
other, cut sides up`, with `egg halves apart, egg halves on opposite sides, one egg` in `--no`.
The general point: when a positional instruction fights the dish, move the instruction to a
different component rather than dropping it.
